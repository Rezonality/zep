...
TEST(BufferTest, GetBlock)
{
    ...
    auto block = pBuffer->GetBlock(SearchType::WORD | SearchType::Word, 1, SearchDirection::Forward);
    ASSERT_EQ(block.firstBlock, 2);
    ASSERT_EQ(block.firstNonBlock, 6);
    ASSERT_EQ(block.secondNonBlock, 9);
   
    pBuffer->SetText("one two three");
    block = pBuffer->GetBlock(SearchType::WORD | SearchType::Word, 1, SearchDirection::Forward);
